"""MCP (Model Context Protocol) JSON-RPC Interceptor for Vinci ADR Daemon.

Provides real-time security interception for MCP servers and clients,
inspecting all `tools/call` JSON-RPC requests and enforcing Vinci ADR policies
...
class VinciMCPMiddleware:
    ...
    the Vinci ADR Daemon, and either permits request forwarding or returns
    an immediate JSON-RPC security error.

    Usage:
        mcp_guard = VinciMCPMiddleware(daemon)
    ...
    def intercept_jsonrpc_dict(self, request: dict[str, Any]) -> tuple[bool, dict[str, Any]]:
        ...
        # Intercept tool call with VinciDaemon
        ...
        logger.warning(
            "MCP tool call blocked by Vinci ADR",
            tool=tool_name,
        ...
        error_resp = create_jsonrpc_error_response(
            ...
            message=f"Vinci ADR Security Policy Violation: {result.reason}",
            data={
                "tool": tool_name,
                "decision": result.decision.value,
                "reason": result.reason,
    ...
    def wrap_handler(
        ...
        Returns:
            Protected handler function checking Vinci ADR before execution.
        """
        ...
        def protected_handler(tool_name: str, arguments: dict[str, Any]) -> Any:
            ...
            raise PermissionError(
                f"Vinci ADR MCP Interceptor blocked tool '{tool_name}': {result.reason}"
            )
...
# Compatibility alias
AegisMCPMiddleware = VinciMCPMiddleware
