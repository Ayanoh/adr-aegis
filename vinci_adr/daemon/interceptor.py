...
The VinciDaemon sits between an AI agent and its tools, intercepting every
tool call and validating it through Vinci ADR before allowing execution.
"""
...
class InterceptionResult:
    ...
    Attributes:
        ...
        evaluation: Full Vinci ADR evaluation result.
...
@dataclass
class DaemonConfig:
    """Configuration for the Vinci ADR Daemon.

    Attributes:
        engine_config: Configuration for the underlying Vinci ADR Engine.
...
class VinciDaemon:
    ...
    Vinci ADR before allowing it to proceed. It supports:
    ...
    def __init__(self, config: DaemonConfig | None = None) -> None:
        """Initialize the Vinci ADR Daemon.

        Args:
            config: Daemon configuration. Uses defaults if None.
        """
        self.config = config or DaemonConfig()
        self._engine = VinciADREngine(self.config.engine_config)
        self._call_count = 0
        self._block_count = 0
        logger.info(
            "Vinci ADR Daemon initialized",
    ...
    def _serialize_input(self, tool_input: dict[str, Any]) -> str:
        """Convert tool input to text for Vinci ADR analysis.
    ...
    def intercept(
        ...
        # Run Vinci ADR analysis on the tool call
        text_to_analyze = f"Tool: {tool_name}\n{self._serialize_input(tool_input)}"
        evaluation = self._engine.evaluate(text_to_analyze)

        # Map Vinci ADR decision to daemon decision
        engine_decision = evaluation.verdict.decision

        if engine_decision == ActionDecision.BLOCK:
            decision = InterceptionDecision.BLOCK
            self._block_count += 1
        elif engine_decision == ActionDecision.ASK:
            if self.config.strict_mode:
                decision = InterceptionDecision.BLOCK
                self._block_count += 1
            else:
                decision = InterceptionDecision.ESCALATE
        elif engine_decision == ActionDecision.SANITIZE:
    ...
    def wrap_tool(
        ...
        def wrapped(**kwargs: Any) -> Any:
            ...
            raise PermissionError(f"Vinci ADR Daemon blocked tool '{name}': {result.reason}")

        return wrapped


# Compatibility alias
AegisDaemon = VinciDaemon
